...
if __name__ == "__main__":
    ...
    pnf = PerlinNoiseFactory(2, octaves=4, tile=(size, size))

    for x in range(size):
        for y in range(size):
            noise[x,y] = pnf(x/size,y/size)
    ...
    # plt.subplot(1, 2, 2)
    # plt.contourf(noise, fields)
    # plt.show()

    print(minn, maxn, step, fields)

    tile_size = 5
    asize = int(size/tile_size)
    tiles = np.zeros((asize, asize))
